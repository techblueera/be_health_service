...
import { 
    ...
    getAllProductsAdmin,
    getSimilarProducts
} from '../../controllers/medicalStore/product.controller.js';
 ...
 *         schema:
 *           type: number
 *         description: Optional minimum average selling price for the product variants.
 *       - in: query
 *         name: maxPrice
 *         schema:
 *           type: number
 *         description: Optional maximum average selling price for the product variants.
 *     responses:
 *       200:
 ...
 *                                     avgSellingPrice:
 *                                       type: number
 *                                       description: The average selling price for this variant across all available inventories in the given pincode.
 *                                     inventory:
 *                                       type: array
...
/**
 * @swagger
 * /api/ms/products/user/similar:
 *   post:
 *     summary: "[User] Get similar products based on a list of product IDs"
 *     tags: [Products]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *                   type: string
 *                   format: objectId
 *                 description: An array of product IDs to find similar products for.
 *                 example: ["60d0fe4f5311236168a109cb", "60d0fe4f5311236168a109cc"]
 *               pincode:
 *                 type: string
 *                 description: The pincode to check for inventory availability. This is mandatory.
 *                 example: "110001"
 *     responses:
 *       200:
 *         description: A list of up to 10 similar products available in the pincode. The response format is identical to the /user/search endpoint.
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                       type: integer
 *       400:
 *         description: Bad Request - Pincode or productIds are missing or invalid.
 *       401:
 *         description: Not authorized.
 *       500:
 *         description: Internal Server Error.
 */
router.post('/user/similar', protect, getSimilarProducts);

/**
 * @swagger
 * /api/ms/products/search:
 *   get:
 ...
 *           default: all
 *         description: "Filter products by category status. 'valid' for products with a valid category, 'invalid' for products with a missing or invalid category, 'all' for all products. This parameter takes precedence over 'categoryId' and 'key'."
 *     responses:
 *       200:
 ...
 *           enum: [all, valid, invalid]
 *           default: all
 *         description: "Filter products by category status. 'valid' for products with a valid category, 'invalid' for products with a missing or invalid category, 'all' for all products."
 *     responses:
 *       200:
...
/**
 * @swagger
 * /api/ms/products/{productId}/variants:
 *   post:
...
/**
 * @swagger
 * /api/ms/products/variants/{variantId}:
 *   put:
...
/**
 * @swagger
 * /api/ms/products/variants/change-requests:
 *   get:
...
/**
 * @swagger
 * /api/ms/products/variants/change-requests/{requestId}/approve:
 *   post:
...
/**
 * @swagger
 * /api/ms/products/variants/change-requests/{requestId}/reject:
 *   post:
