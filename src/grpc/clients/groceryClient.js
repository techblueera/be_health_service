...
const PROTO_PATH = path.join(__dirname, "../protos/grocery.proto");
...
const groceryProto = grpc.loadPackageDefinition(packageDefinition).grocery_service;
...
class GroceryClient {
  constructor(
    ...
    this.client = new groceryProto.GroceryService(
      serverAddress,
    ...
    logger.info(`gRPC Grocery client connected to ${serverAddress}`, 'GroceryClient');
  }

  _setupCircuitBreakerEvents(methodName, breaker) {
    breaker.on('open', () => logger.warn(`Circuit breaker OPEN for ${methodName}`, 'GroceryClient'));
    breaker.on('halfOpen', () => logger.info(`Circuit breaker HALF-OPEN for ${methodName}`, 'GroceryClient'));
    breaker.on('close', () => logger.info(`Circuit breaker CLOSED for ${methodName}`, 'GroceryClient'));
    breaker.on('fallback', () => logger.warn(`Circuit breaker FALLBACK for ${methodName}`, 'GroceryClient'));
  }
  ...
  async updateOrderStatus(orderId, status) {
    try {
      ...
      logger.error(`updateOrderStatus error for order ${orderId}`, 'GroceryClient', error);
      throw error;
    }
  }
  ...
  async getInventoryDetails(inventoryIds) {
    try {
      ...
      logger.error(`getInventoryDetails error for inventories ${inventoryIds}`, 'GroceryClient', error);
      throw error;
    }
  }
  ...
  async getProductDetailsByVariantId(variantIds) {
    try {
      ...
      logger.error(`getProductDetailsByVariantId error for variants ${variantIds}`, 'GroceryClient', error);
      throw error;
    }
  }
...
export const updateOrderStatus = (orderId, status) => groceryClient.updateOrderStatus(orderId, status);
export const getInventoryDetails = (inventoryIds) => groceryClient.getInventoryDetails(inventoryIds);
export const getProductDetailsByVariantId = (variantIds) => groceryClient.getProductDetailsByVariantId(variantIds);


export { GroceryClient as default };
