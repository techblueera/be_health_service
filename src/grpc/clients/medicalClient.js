...
const PROTO_PATH = path.join(__dirname, "../protos/medical.proto");
...
const medicalProto = grpc.loadPackageDefinition(packageDefinition).medical_service;
...
class MedicalClient {
  constructor(
    ...
    this.client = new medicalProto.MedicalService(
      serverAddress,
    ...
    logger.info(`gRPC Medical client connected to ${serverAddress}`, 'MedicalClient');
  }

  _setupCircuitBreakerEvents(methodName, breaker) {
    breaker.on('open', () => logger.warn(`Circuit breaker OPEN for ${methodName}`, 'MedicalClient'));
    breaker.on('halfOpen', () => logger.info(`Circuit breaker HALF-OPEN for ${methodName}`, 'MedicalClient'));
    breaker.on('close', () => logger.info(`Circuit breaker CLOSED for ${methodName}`, 'MedicalClient'));
    breaker.on('fallback', () => logger.warn(`Circuit breaker FALLBACK for ${methodName}`, 'MedicalClient'));
  }
  ...
  async updateOrderStatus(orderId, status) {
    try {
      ...
      logger.error(`updateOrderStatus error for order ${orderId}`, 'MedicalClient', error);
      throw error;
    }
  }
  ...
  async getInventoryDetails(inventoryIds) {
    try {
      ...
      logger.error(`getInventoryDetails error for inventories ${inventoryIds}`, 'MedicalClient', error);
      throw error;
    }
  }
  ...
  async getProductDetailsByVariantId(variantIds) {
    try {
      ...
      logger.error(`getProductDetailsByVariantId error for variants ${variantIds}`, 'MedicalClient', error);
      throw error;
    }
  }
...
const medicalClient = new MedicalClient();

export const updateOrderStatus = (orderId, status) => medicalClient.updateOrderStatus(orderId, status);
export const getInventoryDetails = (inventoryIds) => medicalClient.getInventoryDetails(inventoryIds);
export const getProductDetailsByVariantId = (variantIds) => medicalClient.getProductDetailsByVariantId(variantIds);


export { MedicalClient as default };
