import { Product, ProductVariant, Category, Inventory, ProductVariantChangeRequest } from '../../models/medicalModels/index.js';;
...
const searchProducts = async (req, res) => {
    try {
        const { categoryId, key, searchTerm, pincode, page = 1, limit = 10, categoryStatus = 'all' } = req.query;
        ...
        if (categoryStatus === 'valid') {
            ...
            matchStage.category = { $in: validCategoryIds };
        } else if (categoryStatus === 'invalid') {
            const validCategoryIds = await Category.find().distinct('_id');
            matchStage.category = { $nin: validCategoryIds };
...
const searchProductsForUser = async (req, res) => {
    try {
        const { pincode, key, searchTerm, page = 1, limit = 10, minPrice, maxPrice } = req.query;
                            ...
                            vars: {
                                allBatches: {
                                    $reduce: {
                                        ...
                                        initialValue: [],
                                        in: { $concatArrays: ["$$value", "$$this.batches"] }
                                    }
                                }
                            },
                            in: { $avg: "$$allBatches.sellingPrice" }
                        }
                    }
                }
            },
...
const getSimilarProducts = async (req, res) => {
    try {
        ...
        if (!productIds || !Array.isArray(productIds) || productIds.length === 0) {
            return res.status(400).json({ message: 'productIds must be a non-empty array of product IDs.' });
        }
        ...
        const baseProducts = await Product.find({ _id: { $in: objectIds } }).select('category').lean();
        const categoryIds = [...new Set(baseProducts.map(p => p.category))];

        const getProductsPipeline = (matchConditions, limit) => {
            let pipeline = [
                ...
                { $sample: { size: limit } } // Use sample for variety
            ];
            return pipeline;
        };
        
        // 1. Find similar products available in the pincode
        const similarMatch = {
            category: { $in: categoryIds },
            _id: { $nin: objectIds }
        };
        const similarProducts = await Product.aggregate(getProductsPipeline(similarMatch, limitNum));

        let finalResults = similarProducts;
        
        // 2. If not enough similar products, find any other available products
        if (finalResults.length < limitNum) {
            const needed = limitNum - finalResults.length;
            const foundIds = finalResults.map(p => p._id);
            const otherMatch = {
                _id: { $nin: [...objectIds, ...foundIds] }
            };
            const otherProducts = await Product.aggregate(getProductsPipeline(otherMatch, needed));
            finalResults.push(...otherProducts);
        }
        ...
        logger.error('Error finding similar products for user', 'getSimilarProducts', error);
        res.status(500).json({ message: 'Error finding similar products', error: error.message });
    }
};


export { createProductAdmin, updateProductAdmin, searchProducts, createProductVariant, updateProductVariant, deleteProductVariant, getChangeRequests, approveChangeRequest, rejectChangeRequest, getProductById, searchProductsForUser, getAllProductsAdmin, getSimilarProducts };
