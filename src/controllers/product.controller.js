...
const createProductAdmin = async (req, res) => {
  ...
  try {
    if (!productData || !variantData) {
      return res
        .status(400)
        .json({ message: "productData and variantData are required." });
    }
    ...
    try {
      ...
      return res
        .status(400)
        .json({
          message: `Invalid JSON format in productData or variantData. ${e.message}`,
        });
    }

    const category = await Catalog.findById(
      parsedProductData.catalogNodeId
    ).session(session);
    if (!category) {
      return res
        .status(404)
        .json({
          message: `Catalog with id ${parsedProductData.catalogNodeId} not found.`,
        });
    }
    ...
    if (uploadedImageUrls.length > 0) {
      logger.info(
        "Cleaning up orphaned S3 images due to transaction failure.",
        "createProductAdmin"
      );
    ...
    if (error.code === 11000) {
      return res
        ...
        .json({
          message:
            "A product or variant with the same unique fields (e.g., SKU, barcode) already exists.",
        });
    }
    res
      .status(500)
  ...
  try {
    ...
    if (!productData && !variantsData) {
      return res
        ...
        .json({
          message:
            "At least one of productData or variantsData must be provided.",
        });
    }
    ...
    if (productData) {
      try {
        ...
        return res
          .status(400)
          .json({ message: "Invalid JSON format in productData." });
      }
    }
    if (variantsData) {
      try {
        ...
        return res
          .status(400)
          .json({
            message: `Invalid JSON format in variantsData. ${e.message}`,
          });
      }
    }
    ...
    if (productData) {
      ...
      if (
        parsedProductData.imagesToRemove &&
        Array.isArray(parsedProductData.imagesToRemove)
      ) {
        product.images = product.images.filter((img) => {
          const shouldRemove = parsedProductData.imagesToRemove.includes(
            img.url
          );
    ...
    // --- Update Variants ---
    if (variantsData) {
      const incomingVariantIds = parsedVariantsData
        .map((v) => v._id)
        .filter((id) => id);
      ...
      for (const variant of variantsToDelete) {
        ...
        if (invCount > 0) {
          throw new Error(
            `Cannot delete variant ${variant.variantName} (${variant.sku}) as it has existing inventory.`
          );
        }
      ...
      for (let i = 0; i < parsedVariantsData.length; i++) {
        ...
        if (variantData._id) {
          ...
          if (!variant)
            throw new Error(`Variant with id ${variantData._id} not found.`);
          ...
          if (
            variantData.imagesToRemove &&
            Array.isArray(variantData.imagesToRemove)
          ) {
    ...
    if (imagesToDelete.length > 0) {
      logger.info(
        "Cleaning up old S3 images after successful update.",
        "updateProductAdmin"
      );
    ...
    res
      ...
      .json({
        ...
        data: { ...updatedProduct.toObject(), variants: updatedVariants },
      });
  } catch (error) {
    await session.abortTransaction();
    session.endSession();
    ...
    if (uploadedImageUrls.length > 0) {
      logger.info(
        "Cleaning up newly uploaded S3 images due to transaction failure.",
        "updateProductAdmin"
      );
      for (const url of uploadedImageUrls) {
        await deleteFromS3(url);
    ...
    logger.error(
      `Error updating product ${productId} (admin)`,
      "updateProductAdmin",
      error
    );
...
const searchProducts = async (req, res) => {
  try {
    ...
    if (categoryId) {
      ...
      const requestedCategory = await Catalog.findOne({
        key: key.toUpperCase(),
      });
    ...
    if (startCategoryId) {
      ...
      if (categoryHierarchy.length > 0) {
        ...
        // Fallback for a valid ID that might not be in the hierarchy aggregation for some reason
...
const createProductVariant = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      return res
        .status(400)
        .json({ message: "Invalid JSON format in variantData." });
    }
    ...
    if (parsedVariantData.pricing && parsedVariantData.pricing.length > 0) {
      ...
      if (!firstPricing.pincode || !firstPricing.cityName) {
        ...
        if (
          anotherVariant &&
          anotherVariant.pricing &&
          anotherVariant.pricing.length > 0
        ) {
    ...
    if (error.code === 11000) {
      return res
        ...
        .json({
          message:
            "A variant with the same unique fields (e.g., SKU, barcode) already exists.",
        });
    }
    res
      .status(500)
...
const updateProductVariant = async (req, res) => {
  ...
  try {
    ...
    if (!variant) {
      return res
        .status(404)
        .json({ message: `ProductVariant with id ${variantId} not found.` });
    }
    ...
    if (req.files && Object.keys(req.files).length > 0) {
      return res
        ...
        .json({
          message:
            "This endpoint does not support file uploads for image updates.",
        });
    }
    ...
    logger.error(
      `Error processing update for product variant ${variantId}`,
    ...
    if (error.code === 11000) {
      return res
        ...
        .json({
          message:
            "A variant with the same unique fields (e.g., SKU, barcode) may already exist. This will be checked upon approval.",
        });
    }
    res
      .status(500)
      .json({
        message: "Error processing product variant update request",
        error: error.message,
      });
  }
};
  ...
  try {
    ...
    if (inventoryCount > 0) {
      ...
      return res
        ...
        .json({
          message:
            "Cannot delete variant with existing inventory. Please clear inventory first.",
        });
    }
    ...
    if (imageUrls.length > 0) {
      logger.info(
        `Cleaning up S3 images for deleted variant ${variantId}`,
...
const getChangeRequests = async (req, res) => {
  ...
  if (user.role !== "admin") {
    return res
      ...
      .json({
        message:
          "Forbidden: You do not have permission to view change requests.",
      });
  }
...
const approveChangeRequest = async (req, res) => {
  ...
  if (user.role !== "admin") {
    return res
      .status(403)
      .json({
        message: "Forbidden: You do not have permission to approve requests.",
      });
  }
  ...
  try {
    const request =
      await ProductVariantChangeRequest.findById(requestId).session(session);
    ...
    if (request.status !== "pending") {
      ...
      return res
        .status(400)
        .json({ message: `This request is already ${request.status}.` });
    }

    const variant = await ProductVariant.findById(request.variant).session(
      session
    );
    if (!variant) {
      request.status = "rejected";
      request.rejectionReason =
        "The associated product variant no longer exists.";
      ...
      return res
        ...
        .json({
          message:
            "Associated product variant not found. The request has been automatically rejected.",
        });
    }
    ...
    res
      ...
      .json({
        message:
          "Change request approved and product variant updated successfully.",
        variant,
      });
  } catch (error) {
    await session.abortTransaction();
    session.endSession();
    logger.error(
      `Error approving change request ${requestId}`,
    ...
    if (error.code === 11000) {
      return res
        ...
        .json({
          message:
            "Update failed due to a conflict. A different variant may already have the same unique value (e.g., SKU, barcode).",
        });
    }
    res
      .status(500)
      .json({
        message:
          "An internal error occurred while approving the change request.",
        error: error.message,
      });
  }
};
...
const rejectChangeRequest = async (req, res) => {
  ...
  if (user.role !== "admin") {
    return res
      .status(403)
      .json({
        message: "Forbidden: You do not have permission to reject requests.",
      });
  }
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Change request has been rejected.", request });
  } catch (error) {
    logger.error(
      `Error rejecting change request ${requestId}`,
      "rejectChangeRequest",
      error
    );
    res
      .status(500)
      .json({
        message: "An internal error occurred while rejecting the request.",
        error: error.message,
      });
  }
};
...
const getProductById = async (req, res) => {
  try {
    ...
    logger.error(
      `Error fetching product by ID: ${req.params.productId}`,
      "getProductById",
      error
    );
    res
...
const searchProductsForUser = async (req, res) => {
  try {
    ...
    if (!pincode) {
      return res
        .status(400)
        .json({ message: "Pincode is required for this search." });
    }
    ...
    logger.error(
      "Error searching products for user",
...
const getAllProductsAdmin = async (req, res) => {
  try {
    ...
    logger.error(
      "Error getting all products for admin",
      "getAllProductsAdmin",
      error
    );
    res
...
const getPostedOfferingsByCategoryNode = async (req, res) => {
  try {
    const { categoryNodeId } = req.params;

    /* ---------- Guard ---------- */
    if (!mongoose.Types.ObjectId.isValid(categoryNodeId)) {
      return res.status(400).json({
        message: "Invalid category node ID format",
      });
    }
    ...
    /* ---------- Aggregation ---------- */
    const offerings = await mongoose.connection
      .collection("offerings")
      .aggregate([
        {
          $match: {
            catalogNodeId: new mongoose.Types.ObjectId(categoryNodeId),
            isActive: true,
          },
        },
        {
    ...
    if (!offerings.length) {
      return res.status(200).json({
        message: "No posted offerings found for this category",
        count: 0,
        data: [],
      });
    }
    ...
    logger.error(
      `Error fetching offerings with variants for category node ${req.params.categoryNodeId}`,
